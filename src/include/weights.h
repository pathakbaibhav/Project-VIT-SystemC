 ...
 */
inline Eigen::MatrixXf getWeights(const std::string& file_path, const int dim1, const int dim2) {
    // Open the CSV file
 ...
 */
inline Eigen::VectorXf getWeights(const std::string& file_path, const int dim) {
    // Open the CSV file
    std::ifstream file(file_path);
    if (!file.is_open()) {
        throw std::runtime_error("Failed to open CSV file: " + file_path);
